...
def assign_targets(
    ...
    # Убираем лишние batch-измерения: (1, N, 6) → (N, 6)
    while anchors.dim() > 2:
    ...
    # Regression targets для positives
    reg_targets = torch.zeros(N, 4, device=device)
    pos_mask = labels == POS
    if pos_mask.any():
...
def _balanced_sample(labels: Tensor, n_pos: int, n_neg: int) -> Tensor:
    ...
    if len(pos_idx) > n_pos_actual:
        pos_idx = pos_idx[
            torch.randperm(len(pos_idx), device=labels.device)[:n_pos_actual]
        ]
    if len(neg_idx) > n_neg_actual:
        neg_idx = neg_idx[
            torch.randperm(len(neg_idx), device=labels.device)[:n_neg_actual]
        ]
