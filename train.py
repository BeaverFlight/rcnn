...
Supports v1 (TreeRCNN) and v2 (TreeRCNNV2) via cfg.model_version.
Jupyter usage (unchanged):
    cmd = [sys.executable, 'train.py', '--config', cfg_path,
           '--data_root', TRAIN_DIR, '--out_dir', CKPT_DIR, '--fold', '0']

Checkpoint policy:
  latest.pth   — always the most recent
  best.pth     — epoch with highest F1
  epoch_NNNN.pth + .json — every val_interval epochs
...
def _log_vram(label: str) -> None:
    ...
    logger.info(
        "VRAM [%s]: alloc=%.2f GB  reserved=%.2f GB  total=%.2f GB  (%.0f%%)",
...
def _f1_score(matched: int, detected: int, reference: int) -> float:
    if detected == 0 and reference == 0:
        return 0.0
    p = matched / detected  if detected  > 0 else 0.0
    r = matched / reference if reference > 0 else 0.0
    return 2.0 * p * r / (p + r) if (p + r) > 0 else 0.0


def _quality_score(plot_metrics: list[PlotMetrics]) -> tuple[float, dict]:
    total_det = sum(pm.n_test  for pm in plot_metrics)
    total_ref = sum(pm.n_ref   for pm in plot_metrics)
    total_mat = sum(pm.n_match for pm in plot_metrics)
    p  = total_mat / total_det if total_det > 0 else 0.0
    r  = total_mat / total_ref if total_ref > 0 else 0.0
    f1 = _f1_score(total_mat, total_det, total_ref)
    gm = compute_global_metrics(plot_metrics)
    info = {
        "total_detected":  total_det,
        "total_reference": total_ref,
        "total_matched":   total_mat,
        "precision":       round(p,          4),
        "recall":          round(r,          4),
        "f1":              round(f1,         4),
...
def save_checkpoint(model, optimizer, epoch, best_score, path, cfg=None, scaler=None):
    payload = {
        "epoch":                epoch,
...
def load_checkpoint(model, optimizer, path, device, scaler=None):
    ckpt = torch.load(str(path), map_location=device)
    missing, unexpected = model.load_state_dict(ckpt["model_state_dict"], strict=False)
    if missing:     logger.warning("Missing keys:    %d %s", len(missing),     missing)
    if unexpected:  logger.warning("Unexpected keys: %d %s", len(unexpected),  unexpected)
    try:
        optimizer.load_state_dict(ckpt["optimizer_state_dict"])
    except (ValueError, KeyError) as exc:
        logger.warning("Optimizer incompatible (%s) — fresh start.", exc)
...
# ---------------------------------------------------------------------------
# Evaluation inside training loop
# ---------------------------------------------------------------------------

def _evaluate_fold(model, val_ds, cfg, device) -> list[PlotMetrics]:
    model.eval()
    ...
    with torch.inference_mode():
        for sample in val_ds:
            ...
            logger.info("Val plot %d: %d pts, %d GT — inference...",
            ...
            logger.info("Val plot %d: %.2fs, boxes=%d",
                        plot_id, time.perf_counter() - t0, len(out["boxes"]))
            ...
            logger.info(
                "Val plot %d: det=%d ref=%d match=%d R=%.1f%% P=%.1f%%",
...
def train_fold(fold_idx, train_ids, val_ids, data_root, cfg, out_dir, device):
    ...
    # ── Фабрика: v1 или v2 через cfg.model_version ──
    ...
    if resume_path.exists():
        start_epoch, best_score = load_checkpoint(
            model, optimizer, resume_path, device, scaler=scaler
        )
    ...
    for epoch in range(start_epoch, cfg.training.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            logger.info("Batch e%d/b%d: pts=%d gt=%d",
                        epoch+1, batch_idx+1, points.shape[0], gt_boxes.shape[0])
            _log_vram(f"e{epoch+1}/b{batch_idx+1} pre")
            ...
            if (batch_idx + 1) % log_interval == 0:
                ...
                logger.info(
                    "Epoch %d | Batch %d | total=%.4f | %s | LR=%.2e",
                    epoch+1, batch_idx+1, total_loss.item(),
                    " ".join(f"{k}={v}" for k, v in loss_parts.items()),
                    optimizer.param_groups[0]["lr"],
                )

            if torch.isnan(total_loss) or torch.isinf(total_loss):
                nan_batches += 1
                logger.warning("Epoch %d batch %d: NaN/Inf — skip (%d total)",
        ...
        if (epoch + 1) % val_interval == 0 and len(val_ds) > 0:
            ...
            info["epoch"]      = epoch + 1
            info["fold"]       = fold_idx
            info["train_loss"] = round(mean_loss, 6)
            info["config"]     = cfg_dict
...
def main() -> None:
    ...
    logger.info("Device: %s | Model version: %s",
                device, getattr(cfg, 'model_version', 'v1'))
